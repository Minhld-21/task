...
import {paymentTypes, paymentActions} from '~reduxCore/reducers';
  ...
  try {
    ...
    if (res.success) {
      ...
      if (result?.success === '01') {
        params?.requestType == 'captureWallet'
          ? onSuccess(result?.results?.deeplink)
          : onSuccessWeb(result?.results?.payUrl);
      } else {
        yield onError();
      }
    } else {
